...
import { Pencil, Wrench, FileText, ArrowLeft, Plus } from "lucide-react";
...
export default function PaginaHistorial(): React.ReactElement {
  ...
  const {
    ...
    listar_historial,
    setEquipoId,
  } = useHistorial((state) => state);
  const [searchParams] = useSearchParams();
  const navigate = useNavigate();
  const rol = ObtenerToken()?.rol;
  const puedeCrearHistorial = [
    "SISTEMA",
    "ADMINISTRADOR",
    "TECNICO",
    "VENDEDOR",
        ...
        <div className="flex items-center justify-between gap-4">
          <div>
            ...
            <p className="text-xs text-gray-500 mt-0.5">
              Seguimiento técnico y registros de intervenciones
            </p>
          </div>
            ...
            <tbody className="divide-y divide-gray-100">
              {listar_historial.length > 0 ? (
                listar_historial.map((item) => (
                  ...
                  >
                    ...
                    <td
                      ...
                      title={item.observaciones_tecnicas}
                    >
                      <span className="text-gray-700">
                        {item.observaciones_tecnicas || "-"}
                      </span>
                    </td>

                    <td className="px-4 py-3.5 whitespace-nowrap">
                      <span
                        className={`inline-block px-2.5 py-0.5 text-[11px] font-semibold rounded-full border ${getBadgeEstado(item.estado)}`}
                      >
                        {item.estado}
                      </span>
                    </td>
                    ...
                    <td className="px-4 py-3.5 whitespace-nowrap">
                      <div className="inline-flex items-center gap-1.5 text-xs text-gray-700 font-medium">
                        ...
                        <span>
                          {item.nombres_usuario} {item.apellidos_usuario}
                        </span>
                      </div>
                    </td>

                    {puedeGestionarEstado(item.estado_id) && (
                ...
                <tr>
                  ...
                  >
                    No se encontraron registros de historial técnico para este
                    equipo.
                  </td>
                </tr>
              )}
